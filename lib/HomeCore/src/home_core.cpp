...
uint8_t HomeCore::encoder() {
  ...
  if (!pinAValue && pinBValue)
    state = 1; // Если при спаде линии А на линии B лог. единица, то вращение в
               // одну сторону
  if (!pinAValue && !pinBValue)
...
int **HomeCore::runCore() {
  ...
  if (lights) {
    ...
    light_0 ? digitalWrite(RELAY_LIGHT_0, ON)
            : digitalWrite(RELAY_LIGHT_0, OFF);
    light_1 ? digitalWrite(RELAY_LIGHT_1, ON)
...
unsigned int HomeCore::waterCycleGet() { return waterCycles; }
