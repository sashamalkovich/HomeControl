...
void Functions::sendParam()
{
    String temp;
    const char s = ',';
    temp = '[' + String(pTime->gettime("H:i:s")) + s + String(pParamArray[1][0]) + s + String(pParamArray[1][1]) + s + String(pParamArray[0][0]) + s + String(pParamArray[0][1]) + s + String(lights) + s + String(light_0) + s + String(light_1) + s + String(light_2) + s + String(drainON) + s + String(drFeedback) + ']';
    SERIAL.println(temp);
    Serial.println(temp);
}
